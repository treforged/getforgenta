  ...
  const projections = useMemo(() => {
    ...
    for (let i = 0; i < PROJECTION_MONTHS; i++) {
      ...
      const ccEngRevBalEnd = (cardProjectionData?.simCards ?? []).reduce((s, c) => {
        ...
        const bal = Math.max(0, cardProjectionData?.monthlyRevolvingBalances?.get(c.id)?.[i] ?? 0);
        if (i >= 12 && i <= 16 && bal > 0) {
          console.log(`[card debug] i=${i} card="${c.nickname || c.name || c.id}" revBal0=${revBal0.toFixed(0)} bal[${i}]=${bal.toFixed(0)}`);
        }
        return s + bal;
      }, 0);
      ...
      if (!ccDebtFreeFired) {
        ...
        const virtualRevBalBefore = virtualRevBal;
        virtualRevBal = Math.max(0, virtualRevBal - revolvingPayment + interestAndNewPurchases);
        prevCcEngRevBalEnd = ccEngRevBalEnd;
        if (i >= 11 && i <= 15) {
          console.log(`[vRB debug] i=${i} month=${b.monthLabel} prevCcEngRevBal=${prevCcEngRevBalEnd.toFixed(0)} ccEngRevBalEnd=${ccEngRevBalEnd.toFixed(0)} simRevPay=${simRevolvingPayment.toFixed(0)} revPay=${revolvingPayment.toFixed(0)} intNewPurch=${interestAndNewPurchases.toFixed(0)} vRB_before=${virtualRevBalBefore.toFixed(0)} vRB_after=${virtualRevBal.toFixed(0)} finalLiquid=${finalLiquid.toFixed(0)} floor=${b.monthMinSafe.toFixed(0)}`);
        }
        if (!m0AllSettled && !strictSaveUpMonths.has(i) && virtualRevBal > 0 && finalLiquid > b.monthMinSafe) {
          ...
          if (surplus > 0) {
            ...
            if (i >= 11 && i <= 15) {
              console.log(`[vRB debug] i=${i} step3 surplus=${surplus.toFixed(0)} vRB_post_step3=${virtualRevBal.toFixed(0)}`);
            }
          }
        }
      }
