  ...
  const projections = useMemo(() => {
    ...
    for (let i = 0; i < PROJECTION_MONTHS; i++) {
      ...
      if (!ccDebtFreeFired) {
        ...
        if (i <= 20) {
          console.log(`[vRB2] m${i}: pre=${preVRB2.toFixed(0)} revPay=${revolvingPayment.toFixed(0)} intNew=${interestAndNewPurchases.toFixed(0)} step3=${step3Amt.toFixed(0)} post=${virtualRevBal.toFixed(0)} simRevPay=${simRevolvingPayment.toFixed(0)}`);
        }
      }
