...
**RE-SHOT 2026-09-23 after `167f200b`** (checking 2,847 -> 4,231, ask b0822110) and `c389c9af` (engine convergence fix), with `node scripts/capture-store-frames.mjs <dir>`. That script asserts each screen's marker text and that the cookie banner is gone before it saves a frame. All 14 frames replace the earlier ones at the same paths, and git history holds the old set.

## Frames

| File | Screen | What it shows |
|---|---|---|
| `01-dashboard.png` | Dashboard, Overview | Net worth $13,836 ($54,579 assets, $40,743 liabilities); "CREDIT CARDS PAID OFF Dec 2027, 1 yr 3 mo away" |
...
| `04-forecast.png` | Transactions, Forecast tab | "NEXT MILESTONE Dec 2027 - CC Debt Free!", then "Aug 2030: Vacation Fund Complete!" |
...
- `demo-marketing-lines.engine.test.ts` reads the app's own provider since `93ee4587` (ask 16147de8), so its figures now match what /demo computes. Its clock is pinned to 2026-09-03, so its dates can differ from these 09-23 frames (both read Dec 2027 since `167f200b`).
