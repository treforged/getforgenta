 ...
 *
 * `TouchEvent` EXISTS on iOS Safari, so the `typeof TouchEvent === 'undefined'` guard below let it
 * through; `new TouchEvent(...)` then threw `TypeError: Illegal constructor`. The throw escaped a
 * React touch handler, the `ErrorBoundary` wrapping this chart caught it, and the chart vanished —
