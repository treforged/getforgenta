...
function SnapshotTooltip({ active, payload }: any) {
...
export default function MonthlyBudgetSnapshot({
  ...
  const pieData = useMemo(() => {
    const segments = [
      ...
      expectedRemainingExpenses > 0 && { name: 'Expected bills remaining', value: expectedRemainingExpenses,   color: C.expected },
      surplusAmt > 0            && { name: 'Projected surplus',      value: surplusAmt,                 color: C.surplus },
      shortfallAmt > 0          && { name: 'Projected shortfall',    value: shortfallAmt,               color: C.shortfall },
  ...
  const rows: { label: string; value: number; sign: string; colorClass: string }[] = [
    ...
    { label: 'Spent this month',         value: spentSoFar,                 sign: '−',  colorClass: 'text-destructive' },
    { label: 'Expected bills remaining', value: expectedRemainingExpenses,  sign: '−',  colorClass: 'text-orange-400' },
    { label: 'Projected surplus',        value: projectedSurplus,           sign: '=',  colorClass: projectedSurplus >= 0 ? 'text-primary' : 'text-destructive' },
  ];
    ...
    <div className="card-forged p-5">
        ...
        <div className="relative h-56 flex items-center justify-center">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none gap-0.5">
            <span className="text-[10px] text-muted-foreground uppercase tracking-wider leading-none">
              {projectedSurplus >= 0 ? 'Surplus' : 'Shortfall'}
            </span>
            <span
              className={cn(
      ...
      <div className="flex flex-wrap gap-4 mt-4 pt-4 border-t border-border/30">
        {[
          ...
          { label: 'Expected bills', color: C.expected },
          { label: 'Surplus',        color: C.surplus },
        ].map(l => (
