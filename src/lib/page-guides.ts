...
export const PAGE_GUIDES: Record<GuideKey, PageGuide> = {
  ...
  'debt:cards': {
    ...
    sections: [
      ...
      { title: 'Strategies', body: 'Avalanche pays the card with the highest EFFECTIVE rate first, to minimize total interest. That is the rate your next dollar actually saves, which is not always the rate printed on the front of the card: if part of a balance sits in a 0% promo tranche, your payment lands on the expensive part instead, so a card with a high headline APR can rank below a plain card with a lower one. Snowball pays the smallest balance first for early momentum. Both always cover every card\'s minimum first, then put the remaining available cash toward the priority card.' },
      { title: 'When two answers are both correct, we pick the cheaper one for you', body: 'Some money questions have more than one defensible answer. Wherever that is true, this app takes the one that leaves you with more money, and the screen says which it took rather than leaving you to work it out. Avalanche ranking is one: it compares the rate your next dollar really saves, not the headline rate. Spare money shared between a goal and a debt is another: a goal that is already on schedule takes only what its target date needs this month, and the rest goes to the debt, because a dollar against a card charging you interest saves more than a dollar in a fund that is already on pace. This never bends a number to flatter the plan. A lender\'s stated minimum, a statutory cap and the interest you actually accrue are not ours to choose, so they are reported as they are, even when a cheaper-looking figure would read better.' },
