...
import { aggregateByMonth } from '@/lib/scheduling';
...
import { getPrePaycheckNextMonthBills, mergeWithGeneratedTransactions, type EnrichedTransaction } from '@/lib/pay-schedule';
...
/**
 * Shared forecast pipeline. Assembles the full ForecastInputs and runs the pure calculateForecast
 * engine, so BOTH the Forecast page and the Debt Payoff page derive the exact same authoritative
 * projection (and its step-3 revolving surplus) independently — with no navigation dependency.
 *
 * Previously the step-3 extras were written to context only while the Forecast page was mounted, so
 * a cold Debt Payoff load fell back to the raw (un-accelerated) sim. Both pages now call this hook.
 * The derivation logic is moved verbatim from Forecast.tsx (Stage-2 extraction) — identical numbers.
 */
