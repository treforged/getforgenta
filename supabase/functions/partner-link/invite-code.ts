/**
 * Invite-code primitives for the partner-link Edge Function.
 *
 * Split out of index.ts so they can be exercised by vitest: the function body
 * itself needs Deno and cannot run in this repo's test runner, but these four
 * are pure and use only WebCrypto, which Deno and Node both provide. Same
 * bridge the other edge-function tests use (see
 * src/lib/__tests__/akoya-normalize.test.ts).
 *
 ...
 *   - only its SHA-256 ever reaches the database (design §1 — the `share_token`
