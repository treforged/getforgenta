...
export interface NotificationSignals {
  ...
  learnStreak: number;
  /**
   * The hour (0-23) in the USER's timezone, for callers that do not run in it.
   *
   * The push sender sets this from `profiles.timezone`; the app leaves it undefined and keeps
   * the device clock. See the note in `decideNotification`.
   */
  localHour?: number;
...
export function decideNotification(
  ...
  // ⚠️ THE HOUR MUST BE THE USER'S, NOT THE RUNTIME'S. `getHours()` reads the clock of whatever
  // is executing. On a device that is the reader's own clock and is right. On the push sender it
  // is UTC, which would put quiet hours and STREAK_RISK_HOUR in the wrong place for everyone
  // outside it — a "your streak ends tonight" at 2pm in New York, and a 9pm quiet-hours block
  // that starts at 4pm. So a server-side caller passes `localHour`, computed from the user's
  // stored timezone. Absent, this is byte-identical to before: the client never passes it.
  const nowHours = signals.localHour ?? now.getHours();
