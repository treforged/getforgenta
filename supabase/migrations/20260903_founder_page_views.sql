...
-- `treforgedwebsite/supabase/migrations/20260903_founder_page_views.sql`
-- (Ellis), which is where it is maintained. It is the DENOMINATOR for the
-- founder page: `founder_waitlist` counts signups, and a signup count alone
-- cannot tell "nobody visited" from "people visited and did not sign up" —
-- opposite conclusions from an identical number.
--
-- ⚠️ EDIT IT IN `treforgedwebsite`, NOT HERE. It lives in this folder only so
-- that this repo's migrations describe its own database; two copies that drift
-- are how the last divergence started.
--
-- VERIFIED AGAINST THE LIVE PROJECT 2026-09-03 rather than taken on report,
-- because he invited exactly that:
--   columns          day:date, source:text, views:integer  — no address, no IP,
--                    no user agent, no session id. Nothing identifies anyone.
--   bump_founder_view  anon=false authenticated=false, definer=true,
--                      search_path=public, pg_temp
--   table grants to anon/authenticated/PUBLIC: 0
--   RLS: enabled
-- Both his claims hold.
-- ─────────────────────────────────────────────────────────────────────────────
